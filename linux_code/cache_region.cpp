...
std::vector<Point> generatePlaneCoordinates(int totalPlanes = 300, 
    ...
    // 对 cache_region 进行再次处理
    while (!cache_region.empty()) {
        // 获取 cache_region 中的第一个点
        Point cachedPoint = cache_region.back();
        cache_region.pop_back();
        ...
        if (!coordinates.empty() && coordinates.size() % planesPerRow == 0) {
            y += yInterval;
            yTotalIncrement += yInterval;
            times_increase_y++;
        ...
        bool tooClose = false;
        for (const auto& existingPoint : coordinates) {
            if (calculateDistance(cachedPoint, existingPoint) < minSpacing) {
                tooClose = true;
                break;
            }
        }

        // 如果满足条件，添加到 coordinates，否则继续放入 cache_region
        if (!tooClose) {
            coordinates.push_back(cachedPoint);
        } else {
            cache_region.push_back(cachedPoint);
        }
    }
